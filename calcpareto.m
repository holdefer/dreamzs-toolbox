function paretoScores = calcpareto(objScores,varargin)
...
switch modeStr
    case 'Vrugt et al. (2003)'
        paretoScores = calcpareto(objScores,'g') +...
                       calcpareto(objScores,'z');
    ...
    case 'Zitzler & Thiele (1999)'
        ...
        for k=find(isNonDomIO)
            ...
            paretoScores(k,1) = 1 + sum(all(Y>=X,2))/nPoints;
        ...
        for k=1:nPoints
            if ~isNonDomIO(k)
                ...
                IO = all(Y<=X,2) & isNonDomIO(v);
                paretoScores(k,1) = sum(paretoScores(IO,1));
            end
        end
